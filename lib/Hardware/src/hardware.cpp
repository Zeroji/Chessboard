...
uint32_t timeDiff(uint32_t from_ms, uint32_t to_ms) {
    ...
    if (to_ms < from_ms) {
        // Handle millis() reset (when returned value overflow to 0)
...
uint64_t stableBoardFilter(uint64_t p_boardState, uint32_t p_delay /* = STABLE_BOARD_DELAY_MS */) {
    ...
    static uint32_t lastStableBoard_ms = millis();

    static uint64_t lastBoard    = p_boardState;
    static uint32_t lastBoard_ms = millis();

    const uint32_t now_ms = millis();

    if (p_boardState != lastBoard) {
        // New board state received
        if (timeDiff(lastBoard_ms, now_ms) >= p_delay) {
    ...
    if (timeDiff(lastStableBoard_ms, now_ms) >= p_delay) {
